...
def propagate(w, b, X, Y):
    ...
    #cost = (-1/m) * np.sum(np.dot(Y.T,np.log(A))+np.dot((1-Y).T,np.log(1-A)))#None#(1/m)*(np.dot(Y,np.log(A))+np.dot((1-Y),np.log(1-A))#None                                  # compute cost
...
def model(X_train, Y_train, X_test, Y_test, num_iterations = 2000, learning_rate = 0.5, print_cost = False):
    ...
    #'w', 'b', 'X', 'Y', 'num_iterations', and 'learning_rate'
