...
shape_X = X.shape#None
shape_Y = Y.shape#None
...
def layer_sizes(X, Y):
    ...
    """
    ### START CODE HERE ### (≈ 3 lines of code)
    n_x = X.shape[0]#None # size of input layer
    n_h = 4
    n_y = Y.shape[0]#None # size of output layer
    ### END CODE HERE ###
    return (n_x, n_h, n_y)
...
def compute_cost(A2, Y, parameters):
    ...
    logprobs = np.multiply(np.log(A2),Y)+np.multiply(np.log(1-A2),(1-Y))#None
    cost = - np.sum(logprobs)/m#None
    ### END CODE HERE ###
